...
5. Incorporate feedback and save finalized plan to `plans/architect-plan-[topic]-[version].md`
...
1. Read `README.md`, `AGENTS.md`, and any existing `plans/` documents
...
4. Determine the engagement mode from what you find and the user's request
...
Present the draft to the user as a summary before writing the file. Incorporate feedback, then save to a topic/versioned plan file.

## Plan File Naming

Save finalized plans under `plans/` using:

```text
plans/architect-plan-[topic]-[version].md
```

Rules:

  - `[topic]` should be a short lowercase slug using letters, numbers, and hyphens, derived from the project or change name
  - `[version]` should start at `v1` and increment when the plan changes materially: `v2`, `v3`, etc.
  - If an older plan for the same topic exists, read it before creating the new version
  - Do not overwrite an existing version unless the user explicitly asks to revise that exact file
  - Examples: `plans/architect-plan-auth-refactor-v1.md`, `plans/architect-plan-mobile-app-v2.md`
...
STOP IMMEDIATELY if you consider:
  ...
  - Making file edits beyond the selected `plans/architect-plan-[topic]-[version].md` file
  - Running tests, builds, or deployments
  - Generating detailed PRDs with user stories; that is the `prd` skill's job
...
Before finishing, verify:
  ...
  - [ ] Confirmed engagement mode: greenfield / feature / refactor
  ...
  - [ ] Saved to `plans/architect-plan-[topic]-[version].md`
